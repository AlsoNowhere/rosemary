...
const ExampleModel = function(){const m = new modeller.ModelBuilder();m.add("E","boolean");return m.create();}();

describe("ModallerForm2 component - checkbox",()=>{
    describe("a single checkbox field",()=>{
        let component;
        let context;
        ...
        it("should output a checkbox field",()=>{
            expect(context.childAt(0).childAt(0).childAt(0).prop("type")).toBe("checkbox");
        });

        it("should give the field a default value",()=>{
            expect(context.childAt(0).childAt(0).childAt(0).prop("checked")).toBe(false);
        });
    });
    ...
    describe("adding a pre value to the form",()=>{
        ...
        it("should give the field the assigned value",()=>{
            expect(context.childAt(0).childAt(0).childAt(0).prop("checked")).toBe(exampleValue);
        });
    });
});
